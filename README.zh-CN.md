...
- `music-agent` 只是一个本地控制工具，本身不提供音乐曲库、账号体系或托管式流媒体服务。
- 播放可用性依赖 YouTube、Bilibili、SoundCloud、KKBOX 等第三方平台；上游平台的地区限制、下架、接口变更或策略调整都可能随时影响结果。
- 使用者需要自行遵守相关音源平台的服务条款、版权规则以及你所在地区适用的法律法规。
- 本项目仅面向个人、合法用途；请不要用它分发受版权保护的音频、绕过平台限制，或构建未经授权的商业音乐服务。
- 本仓库维护者不拥有第三方音频、封面、榜单数据或相关元数据的版权。
